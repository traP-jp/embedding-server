...
class OcrEngine:
    ...
    def read_image_text(self, image_path: str) -> str:
        ...
        lines: list[str] = []
        for word_prediction in getattr(results, "words", []):
            rec_score = float(getattr(word_prediction, "rec_score", 0.0))
            det_score = float(getattr(word_prediction, "det_score", 0.0))
            if rec_score < self.config.ocr_rec_threshold or det_score < self.config.ocr_det_threshold:
                continue

            text = str(getattr(word_prediction, "content", "")).strip()
            if not text:
                continue
